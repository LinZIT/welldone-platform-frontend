...
import { MenuItem, darken } from "@mui/material";

import AddCircleOutlineRounded from "@mui/icons-material/AddCircleOutlineRounded";
import CloseRounded from "@mui/icons-material/CloseRounded";

import { FormikState, Formik, Form } from "formik";
import { TypographyCustom, TextFieldCustom, ButtonCustom, SelectCustom, CalendarCustom } from "../../custom";
import { IClient, IAdjustingCompany, IInsuranceCompany, IAdviser, ITeam } from "../../../interfaces";
...
import { ModalMultipleSelector } from "../../ui/content/ModalMultipleSelector";
...
export const ClaimDialog: FC<ClaimDialogProps> = ({ client, getClaims }) => {
    ...
    const onSubmit = async (values: InitialValues, resetForm: (nextState?: Partial<FormikState<InitialValues>> | undefined) => void) => {
        ...
        body.append('claim_date', String(moment(claimDate).format('YYYY-MM-DD')));
    ...
    return (
        <>
            <IconButton onClick={openModal}>
                <AddCircleOutlineRounded color={'success'} />
            </IconButton>
            <Dialog fullScreen open={open} id="form-modal">
                ...
                <Box sx={{ margin: 'auto', width: '80%', minHeight: '100vh', mt: 15 }}>
                    <DescripcionDeVista title={"Agregar Claim"} description={`Aqui podras agregar un nuevo claim al cliente seleccionado (${client?.names} ${client?.surnames}), en la esquina superior derecha puedes cerrar el modal y volver a la informacion del cliente.`} buttons={false} />
                    <Formik
