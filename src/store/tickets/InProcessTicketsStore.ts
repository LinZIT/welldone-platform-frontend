...
interface State {
    ...
    setTicket: (ticket: ITicket, status: 'Terminado' | 'Abierto' | 'Cancelado') => void;
...
export const useInProcessTicketStore = create<State>((set, get) => ({
    pagination: {
        status: false,
        data: {
            current_page: 0,
    ...
    getTickets: async () => {
        const { status, response, err }: { status: number, response: any, err: any } = await request(`/ticket/get/in_process`, 'GET');
        switch (status) {
            case 200:
    ...
    setTicket: (ticket: ITicket, status: 'Terminado' | 'Abierto' | 'Cancelado') => {
